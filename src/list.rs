...
impl ReList {
    ...
    pub fn len(&self) -> Value {
        let len = self.list.read().unwrap().len() as i64;
        Value::Integer(len)
    }

    pub fn push_front(&self, val_iter: IntoIter<Value>) -> Value {
        self.push_inner(val_iter, true)
    }

    pub fn push_back(&self, val_iter: IntoIter<Value>) -> Value {
        self.push_inner(val_iter, false)
    }

    pub fn pop_front(&self, cnt: i64) -> Value {
        self.pop_inner(cnt, true)
    }

    pub fn pop_back(&self, cnt: i64) -> Value {
        self.pop_inner(cnt, false)
    }

    pub fn block_pop_front(&self, timeout_in_sec: f64) -> Value {
        self.block_pop_inner(timeout_in_sec, true)
    }

    pub fn block_pop_back(&self, timeout_in_sec: f64) -> Value {
        self.block_pop_inner(timeout_in_sec, false)
    }

    pub fn range(&self, mut start: i64, mut end: i64) -> Value {
        let mut ans = Vec::new();
...
impl ReList {
    fn push_inner(&self, val_iter: IntoIter<Value>, is_front: bool) -> Value {
        let len = {
    ...
    fn pop_inner(&self, mut cnt: i64, is_front: bool) -> Value {
        let mut list_guard = self.list.write().unwrap();
    ...
    fn block_pop_inner(&self, mut timeout_in_sec: f64, is_front: bool) -> Value {
        ...
        loop {
            ...
            if let Some(value) = value {
                ...
                ans.push(Value::BulkStrings(self.name.clone()));
                ans.push(value);
                return Value::Arrays(ans);
            }
